...
<?php
    ...
    if(isset($_POST["btnSubmit"])){
        ...
        $sql = "SELECT * FROM lecturer WHERE epf_no='$username' AND password='$password'";
        ...
        if($row =mysqli_fetch_array($ret)){
            ...
            $sql2 = "SELECT * FROM batch WHERE course_director='$row[1]'";
            $ret2 = mysqli_query($con,$sql2);

            if($row2 =mysqli_fetch_array($ret2)){
                header("Location: http://localhost/FInal%20Project/dashboard/courseDirector/");
            }
            else{
                header("Location: http://localhost/FInal%20Project/dashboard/lecturer/");
            }
